...
public class DynamicList<T>
{
	...
	public void Add(T item)
	{
		if(this.head == null)
		{
			// Create a new head and tail if the list is empty
			this.head = new ListNode(item);
			this.tail = this.head;
		}
		else
		{
			// Append the item after the tail if the list is not empty
	...
	// I created this method. It inserts an element into the specified index
	public void Insert(int index, T item)
	{
		...
		// Find the element currently occupying the specified index
		int currentIndex = 0;
		...
		ListNode newNode = new ListNode(item, prevNode);
	...
	private void RemoveListNode(ListNode node, ListNode prevNode)
	{
		...
		if(count == 0)
		{
			// Remove head and tail and the list becomes empty
			this.head = null;
			this.tail = null;
		}
		else if(prevNode == null)
		{
			// Update the head if it was removed
